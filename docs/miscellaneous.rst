...
specification
<https://github.com/elementrem/wiki/wiki/Elementrem-Contract-ABI>`_.  The
...
- ``keccak256(...) returns (bytes32)``: compute the Elementrem-SHA-3 (Keccak-256) hash of the (tightly packed) arguments
...
- ``<address>.balance`` (``uint256``): balance of the :ref:`address` in Mey
- ``<address>.send(uint256 amount) returns (bool)``: send given amount of Mey to :ref:`address`, returns ``false`` on failure
- ``<address>.transfer(uint256 amount)``: send given amount of Mey to :ref:`address`, throws on failure
...
- ``payable`` for functions: Allows them to receive Element together with a call.
