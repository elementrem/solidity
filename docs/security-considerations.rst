...
of Element hands over control to that contract (B). This makes it possible for B
...
of ``send``, but it still exposes a weakness: Element transfer always
includes code execution, so the recipient could be a contract that calls
back into ``withdraw``. This would let it get multiple refunds and
basically retrieve all the Element in the contract.
...
Note that re-entrancy is not only an effect of Element transfer but of any
...
Sending and Receiving Element
===========================

- Neither contracts nor "external accounts" are currently able to prevent that someone sends them Element.
  Contracts can react on and reject a regular transfer, but there are ways
  to move Element without creating a message call. One way is to simply "mine to"
  the contract address and the second way is using ``selfdestruct(x)``. 

- If a contract receives Element (without a function being called), the fallback function is executed.
  If it does not have a fallback function, the Element will be rejected (by throwing an exception).
...
Restrict the Amount of Element
============================

Restrict the amount of Element (or other tokens) that can be stored in a smart
contract. If your source code, the compiler or the platform has a bug, these
funds may be lost. If you want to limit your loss, limit the amount of Element.
...
are the arguments in range, did they send enough Element, does the person
...
self-checks like "Has any Element leaked?",
