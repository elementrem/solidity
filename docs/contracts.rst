...
API, this is done as follows::
    ...
    var MyContract_ = web3.ele.contract(source);
    MyContract = web3.ele.contract(MyContract_.CONTRACT_NAME.info.abiDefinition);
    // deploy new contract
...
Element (without data).  In such a context, there is usually very little gas available to
...
.. warning::
    Contracts that receive Element but do not define a fallback function
