...
As of today it has a method called `web3.ele.Contract <https://web3js.readthedocs.io/en/1.0/web3-eth-contract.html#new-contract>`_
...
::
    ...
    contract Register is priced, owned {
        ...
        // automatically reject all Element sent to it.
...
Element (without data). Additionally, in order to receive Element, the fallback function
must be marked ``payable``. If no such function exists, the contract cannot receive
Element through regular transactions.
...
.. warning::
    Contracts that receive Element directly (without a function call, i.e. using ``send`` or ``transfer``)
    but do not define a fallback function
    throw an exception, sending back the Element (this was different
    before Solidity v0.4.0). So if you want your contract to receive Element,
    you have to implement a fallback function.

.. warning::
    A contract without a payable fallback function can receive Element as a recipient of a `coinbase transaction` (aka `miner block reward`)
    or as a destination of a ``selfdestruct``.

    A contract cannot react to such Element transfers and thus also cannot reject them. This is a design choice of the EVM and Solidity cannot work around it.
...
::
    ...
    contract Test {
        ...
        // Sending Element to this contract will cause an exception,
    ...
    // This contract keeps all Element sent to it with no way
    // to get it back.
    contract Sink {
        function() payable { }
    }
    ...
    contract Caller {
        function callTest(Test test) {
            ...
            // if someone sends element to that contract,
