...
string getIPCSocketPath()
{
	...
	if (ipcPath.empty())
		BOOST_FAIL("ERROR: ipcPath not set! (use --ipcpath <path> or the environment variable ELE_TEST_IPC)");
...
void ExecutionFramework::sendMessage(bytes const& _data, bool _isCreation, u256 const& _value)
{
	...
	if (!_isCreation)
	{
		...
		BOOST_REQUIRE(m_rpc.ele_getCode(d.to, "latest").size() > 2);
		// Use ele_call to get the output
		m_output = fromHex(m_rpc.ele_call(d, "latest"), WhenError::Throw);
	}

	string txHash = m_rpc.ele_sendTransaction(d);
	m_rpc.test_mineBlocks(1);
	RPCSession::TransactionReceipt receipt(m_rpc.ele_getTransactionReceipt(txHash));
	...
	if (_isCreation)
	{
		...
		string code = m_rpc.ele_getCode(receipt.contractAddress, "latest");
...
void ExecutionFramework::sendElement(Address const& _to, u256 const& _value)
{
	RPCSession::TransactionData d;
	d.data = "0x";
	d.from = "0x" + toString(m_sender);
...
{
	auto latestBlock = m_rpc.ele_getBlockByNumber("latest", false);
	return size_t(u256(latestBlock.get("timestamp", "invalid").asString()));
}

size_t ExecutionFramework::blockTimestamp(u256 _number)
{
	auto latestBlock = m_rpc.ele_getBlockByNumber(toString(_number), false);
	return size_t(u256(latestBlock.get("timestamp", "invalid").asString()));
}
...
{
	string code = m_rpc.ele_getCode(toString(_addr), "latest");
	return !code.empty() && code != "0x";
}

u256 ExecutionFramework::balanceAt(Address const& _addr)
{
	return u256(m_rpc.ele_getBalance(toString(_addr), "latest"));
}

bool ExecutionFramework::storageEmpty(Address const& _addr)
{
	h256 root(m_rpc.ele_getStorageRoot(toString(_addr), "latest"));
	BOOST_CHECK(root);
