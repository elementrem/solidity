...
BOOST_AUTO_TEST_CASE(blockchain)
{
	...
	BOOST_CHECK(m_rpc.rpcCall("miner_setElementbase", {"\"0x1212121212121212121212121212121212121212\""}).asBool() == true);
...
BOOST_AUTO_TEST_CASE(transfer_element)
{
	char const* sourceCode = R"(
		contract A {
			function A() payable {}
...
BOOST_AUTO_TEST_CASE(contracts_as_addresses)
{
	char const* sourceCode = R"(
		contract helper {
			function() payable { } // can receive element
		}
		contract test {
			helper h;
...
// Disabled until https://github.com/elementrem/solidity/issues/715 is implemented
...
BOOST_AUTO_TEST_CASE(send_zero_element)
{
	// Sending zero element to a contract should still invoke the fallback function
	...
	char const* sourceCode = R"(
		contract C {
			modifier tryCircumvent {
				if (false) _; // avoid the function, we should still not accept element
			}
