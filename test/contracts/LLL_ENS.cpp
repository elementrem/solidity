...
BOOST_AUTO_TEST_CASE(create_subnode)
{
	...
	// Set ownership of "ele" sub-node to account(1)
	BOOST_REQUIRE(callContractFunction("setSubnodeOwner(bytes32,bytes32,address)", 0x00, keccak256(string("ele")), ACCOUNT(1)) == encodeArgs());
	...
	BOOST_CHECK(m_logs[0].topics[1] == u256(0x00));
	BOOST_CHECK(m_logs[0].topics[2] == keccak256(string("ele")));

	// Verify that the sub-node owner is now account(1).
	u256 namehash = keccak256(h256(0x00).asBytes() + keccak256("ele").asBytes());
...
BOOST_AUTO_TEST_CASE(create_subnode_fail)
{
	...
	// Send account(1) some element for gas.
	sendElement(account(1), 1000 * element);
	BOOST_REQUIRE(balanceAt(account(1)) >= 1000 * element);

	// account(1) tries to set ownership of the "ele" sub-node.
	m_sender = account(1);
	BOOST_REQUIRE(callContractFunction("setSubnodeOwner(bytes32,bytes32,address)", 0x00, keccak256(string("ele")), ACCOUNT(1)) == encodeArgs());
