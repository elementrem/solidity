...
)
{
	...
	catch(ele::OptimizerException const&)
	{
...
string CompilerStack::computeSourceMapping(ele::AssemblyItems const& _items) const
{
	...
	for (auto const& item: _items)
	{
		...
		int sourceIndex =
			...
			-1;
		char jump = '-';
		if (item.getJumpType() == ele::AssemblyItem::JumpType::IntoFunction)
			jump = 'i';
		else if (item.getJumpType() == ele::AssemblyItem::JumpType::OutOfFunction)
			jump = 'o';
...
Json::Value CompilerStack::gasEstimates(string const& _contractName) const
{
	...
	if (ele::AssemblyItems const* items = assemblyItems(_contractName))
	{
		Gas executionGas = GasEstimator::functionalEstimation(*items);
		u256 bytecodeSize(runtimeObject(_contractName).bytecode.size());
		Gas codeDepositGas = bytecodeSize * ele::GasCosts::createDataGas;
	...
	if (ele::AssemblyItems const* items = runtimeAssemblyItems(_contractName))
	{
		/// External functions
