...
public:
	explicit CompilerContext(CompilerContext* _runtimeContext = nullptr):
		m_asm(std::make_shared<ele::Assembly>()),
	...
	/// Note that the generator should not assume that objects are still alive when it is called,
	/// unless they are guaranteed to be alive for the whole run of the compiler (AST nodes, for example).
	ele::AssemblyItem lowLevelFunctionTag(
		std::string const& _name,
		unsigned _inArgs,
		unsigned _outArgs,
		std::function<void(CompilerContext&)> const& _generator
	...
	ele::AssemblyItem namedTag(std::string const& _name) { return m_asm->namedTag(_name); }
	/// Adds a subroutine to the code (in the data section) and pushes its size (via a tag)
	/// on the stack. @returns the pushsub assembly item.
	ele::AssemblyItem addSubroutine(ele::AssemblyPointer const& _assembly) { return m_asm->appendSubroutine(_assembly); }
	...
	ele::LinkerObject const& assembledObject() const { return m_asm->assemble(); }
	ele::LinkerObject const& assembledRuntimeObject(size_t _subIndex) const { return m_asm->sub(_subIndex).assemble(); }
...
private:
	...
	std::map<std::string, ele::AssemblyItem> m_lowLevelFunctions;
